## Hi, I'm Khushboo 👋

I'm a Staff PM who's spent 10+ years making payments work from
checkout flows and alternative payment methods to fraud prevention
and chargeback operations for retail industries and ecommerce. 

Lately I've been spending my evenings building AI agents that automate
the manual, repetitive parts of payment ops work mostly the kind of work
that burns analyst time without requiring human judgment.

These repos are my personal design lab. No code (I'm a PM, not an
engineer) but plenty of architecture thinking, product decisions,
and system design for anyone curious about AI-native payment tools.

📌 **What I'm building**
- 🛒 [AI Grocery Planner Agent](https://github.com/khushbooleuva/ai-grocery-planner-agent) — multi-constraint weekly optimizer
- 💳 [Chargeback Workflow Agent](https://github.com/khushbooleuva/chargeback-workflow-agent) — HITL dispute management
- 🔍 [Fraud Intelligence Dashboard](https://github.com/khushbooleuva/fraud-intelligence-dashboard) — LLM-powered risk scoring

 · 💼 [LinkedIn](https://linkedin.com/in/khushbooleuva)
